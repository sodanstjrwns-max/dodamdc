...
for (const [name, engine] of [['chromium', chromium], ...(process.env.TEST_WEBKIT ? [['webkit', webkit]] : [])]) {
  ...
  try {
    for (const width of [320, 390, 1440]) {
      ...
      assert.match(await page.locator('#delivery-checklist h2').textContent(), /관리자 페이지 접속 안내/)
      assert.match(await page.locator('#delivery-checklist').textContent(), /아이디 입력이나 최초 계정 생성은 필요하지 않습니다/)
      assert(!(await page.locator('#delivery-checklist').textContent()).includes('기술 점검'))
      ...
      for (const internalCopy of ['원장님께 보낼 안내문', '발송용 안내문', '안내문 전체 복사', '아래 설명서를 먼저 읽어주시면 감사하겠습니다.']) assert(!source.includes(internalCopy), 'Sender-only draft must not be served: ' + internalCopy)
