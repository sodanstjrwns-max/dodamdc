...
## 증상 체크 — 선택형 상담 준비 안내 (2026-09-15)
- 기능: `/symptom-check`에서 부위 1개 → 증상 복수 선택 → 검사에서 확인할 점과 관련 진료 안내. 도담 블루·연그린·Wanted Sans·둥근 카드 디자인, PC/모바일 메뉴와 홈의 별도 진입 링크를 제공합니다. 기존 상황별 카드 5개는 유지합니다.
- 데이터: `src/data/symptom-check.ts`의 7개 부위·34개 증상·13개 안내. 앞니/어금니/잇몸/사랑니/턱관절/입안 전체/아이 치아를 실제 도담 진료 12개와 연결합니다. 미니쉬·교정·안면미용·수면진료 등의 미제공 서비스를 복제하지 않습니다. AI API·DB 테이블·신규 저장 서비스는 없습니다.
- 사용법: 부위를 고르고 ‘증상 선택하기’, 해당 증상을 모두 고른 뒤 ‘안내 확인하기’를 누릅니다. 같은 안내는 중복 표시하지 않으며 다시 선택·처음부터 체크가 가능합니다. 부위를 바꾸면 이전 증상은 초기화됩니다. 결과 링크는 기존 진료 페이지, 도담의 공식 네이버 예약, `/reservation`, 전화입니다. 증상을 예약 URL에 붙이거나 예약 폼에 자동 전달하지 않습니다.
- 안전: 진단·치료 확정·위험 배제 기능이 아닙니다. 호흡/연하 곤란·급속한 얼굴/목 붓기·멈추지 않는 심한 출혈 선택 시 일반 체크를 중단하고 119/응급실을 먼저 안내합니다. 사고로 빠지거나 위치가 바뀐 치아는 즉시 치과/응급기관에 연락하도록 합니다. 당일 문의가 필요한 선택은 결과에서 우선 안내하고 일반 예약 버튼을 숨깁니다. **신규 문항·결과는 의료진 재검토 전인 일반 정보이며 임상 검증을 완료했다는 의미가 아닙니다.**
- 개인정보: 선택은 DOM에서만 사용하고 URL·쿠키·localStorage·sessionStorage·서버에 저장하지 않습니다. 새로고침/페이지 이탈/BFCache 복귀 시 초기화합니다. 이 페이지에는 전환 ticket·GA4·Clarity·외부 beacon·자체 page_views 기록을 발행하지 않습니다. no-store/no-transform/no-referrer 및 CSP connect-src none/script-src self로 자동 분석 삽입과 전송을 차단합니다. 호스팅 사업자의 기본 HTTP 접근 로그까지 없다는 뜻은 아닙니다.
...
- Cloudflare 배포 이력 확인: 정상 사진·줄바꿈 배포 `59461e3e`(07:50 UTC) 이후, 이전 커밋 `05f341a`가 `724dc4ae`(09:19 UTC), `27d556a4`(09:21 UTC)로 다시 production에 배포돼 변경 사항이 사라졌습니다. 누가 어떤 작업창에서 실행했는지는 확인하지 않았습니다.
- GitHub main과 같은 최신 통합본 `efdd7d1`을 다시 빌드하고 사진 파일·줄바꿈 HTML·CSS v18 포함을 확인한 후 `5aed38e8`(https://5aed38e8.seoul-dodam-dental.pages.dev)로 복구했습니다. 진료시간·백과사전 개선도 함께 보존합니다. DB·환자 데이터·운영 비밀번호는 변경하지 않았습니다.
- 복구 후 실제 https://dodamdc.kr/mission 에서 Chromium/WebKit × 390/1440px 네 화면의 사진 두 장·원본 비율·설명 두 문단과 명시적 줄바꿈 두 개·CSS v18·가로 넘침 없음·JS 오류 없음을 확인했습니다. 실제 화면 캡처는 Git 제외 `.artifacts/recovered-live-*.png`에 보관합니다.
- **후속 배포 주의**: 다른 작업 사본에서 과거 커밋을 직접 배포하면 다시 덮어쓸 수 있습니다. 모든 배포 작업은 GitHub main을 동기화한 후 진행해야 합니다. 현재 복구가 다른 작업자의 향후 직접 배포까지 차단하는 것은 아닙니다.
...
- 미션의 큰 한 덩어리 문장을 ‘파란 핵심 문장 → 별도 설명 두 문단’으로 분리했습니다. 여섯 가지 약속·연혁·기존 설명 내용은 유지합니다.
- 공통 공개 본문: 진료 설명·칼럼/공지 본문·개인정보 안내의 `.prose`를 모바일 16px/PC 17px, 행간 1.9, 최대 문장 폭 44rem으로 정리했습니다. 핵심 요약과 설명의 크기/굵기를 구분하고 소제목·문단·목록 간격을 개선했습니다. FAQ, 의료진 이야기, 장비/약속 카드도 읽기 쉽게 조정했습니다. 자동 문장 재작성이나 숨김, 전체 CMS 저장 데이터 변환은 하지 않습니다.
- 홈페이지 고유 블루/연그린·로고·모션·예약 버튼과 현재 핸드오버의 내용·비밀번호/결제 표시는 변경하지 않습니다. 공통 본문 CSS v18. 외부 네이버·구글·카카오 등 플랫폼 정보 수정은 실행하지 않았습니다. 등록용 대표 주소는 https://dodamdc.kr 입니다.
- 작업 중 원격에 추가된 `3c4021a`(백과사전 심화 설명·검색)와 `05f341a`(진료시간·관리자 시간표·진료별 메타 설명)를 보존·병합했습니다. 백과사전은 별도의 환자용 해설 스타일을 유지하며 공통 스타일 파일 v18과 전용 CSS/JS를 함께 로드합니다. 운영 시간표나 환자 데이터를 직접 변경하지 않습니다.
- 병합본 검사: typecheck/build, 모바일 Chromium·Linux WebKit 10그룹(신규 사진 비율/문단 위계 포함), 디자인 80뷰, 예약 동선 8그룹, 보안 11그룹 통과. SEO 701페이지/이미지 38개 오류·경고 0. 추가 원격 기능도 백과사전 513페이지/28뷰·진료시간/메타 12진료·7경로·6뷰 및 격리 관리자 수정/재조회 검사 통과. 실제 iPhone/Kakao 앱의 네이티브 환경 검수와는 구분합니다.
- 운영 배포: 소스 `2a43f80`, 배포 `59461e3e` (https://59461e3e.seoul-dodam-dental.pages.dev), 정식 확인 https://dodamdc.kr/mission. 실주소의 Chromium/Linux WebKit 320·390·1440px에서 새 사진 로딩·전체 비율·핵심 문장/설명 분리·가로 넘침 0을 확인했습니다. 임플란트·진료시간·MTA 백과사전 페이지도 200/정상 레이아웃 확인. 운영 예약 제출·환자정보 접근·DB/R2 변경은 하지 않았습니다.

## 핸드오버 도담 브랜드 스타일 수정 (2026-09-13)
- 참고 페이지의 내용·8개 섹션 순서는 그대로 유지하고, 디자인만 도담 홈페이지의 스타일로 변경했습니다. 도담 블루 `#0069b3`·연그린 `#b9e7a6`·밝은 배경 `#f7f9f5`, 자체 호스팅 Wanted Sans, 실제 로고, 타원형 장식, 둥근 카드·캡슐형 버튼을 적용했습니다.
- 기존 관리자 비밀번호 표시/복사·결제 금액·계좌·지원 약속 및 본문 전체는 변경하지 않습니다. 폰트는 Google 외부 서체에서 홈페이지와 같은 로컬 파일로 전환하고 CSP도 same-origin 서체만 허용합니다. CSS v5 / JS v4.
- 운영 반영 완료: 소스 `4f53c66`, 배포 `2142897c` (https://2142897c.seoul-dodam-dental.pages.dev), 정식 안내서 https://dodamdc.kr/handover. typecheck/build 및 본문 전체 동일성 확인, 로컬/운영 Chromium·Linux WebKit 각각 320·390·1440px에서 도담 색상·로고·로컬 서체·비밀번호 복사·결제·인쇄·가로 넘침·JS 오류 검사를 통과했습니다. 운영 비밀번호는 로그나 QA 이미지에 저장하지 않았습니다.

## 이전 참고 안내서 구성 일치 및 비밀번호 표시 (2026-09-13)
- 사용자가 지정한 연세온 납품 안내서와 같은 820px 문서·네이비/골드 표지·본문·통계 카드·표·관리자 정보·결제 박스·마무리 레이아웃을 적용했습니다. 표지/편지 다음 `CORE PURPOSE → HIGHLIGHTS → SPECS & HIDDEN GEMS → ADMIN GUIDE → IMPORTANT → HOW TO GROW → SUPPORT & FEEDBACK → PAYMENT` 순서입니다.
- 수신인 한휘림·서울도담치과·dodamdc.kr, 실제 운영 기능으로 내용을 맞췄습니다. 운영 사이트맵 701 URL, 백과사전 512개, 지역×진료 156개, 진료 12개, FAQ 252문항을 2026-09-13 기준으로 확인했습니다. 실제 색인 수가 아니며 연세온의 미구현 팝업/Speakable/AI봇 수/색인 성과나 VAT 조건을 복제하지 않았습니다.
- **공개 범위에 관한 사용자의 명시적 요청**: 환자·회원 정보 접근 위험과 누구나 열 수 있는 URL임을 안내한 뒤에도 사용자는 핸드오버에 실제 관리자 비밀번호를 표시하도록 재차 요청했습니다. `/handover`의 관리자 정보 박스에 기존 `ADMIN_PASSWORD`를 서버에서 HTML 이스케이프하여 표시하고 복사 기능을 제공합니다. 비밀번호를 변경하거나 소스·정적 자산·Git·로그·메타데이터에 하드코딩하지 않습니다.
- **중요한 한계**: 정식 `/handover` 열람자는 관리자 비밀번호를 알 수 있습니다. noindex/no-store/no-referrer/no-snippet은 공개 접근을 막는 수단이 아니며 비밀번호가 공개된 상태에서는 로그인 장벽으로 환자정보 접근을 보호한다고 볼 수 없습니다. 문서와 링크를 외부에 공유하지 말라는 안내 및 변경 요청 방법을 포함했습니다. 향후 공개를 철회하려면 표시를 제거하고 비밀번호도 교체해야 합니다.
- 공식 설정 origin의 `/handover`에서 표시합니다. 로컬 localhost는 개발용 설정만 사용하며, immutable Pages·샌드박스 미리보기 호스트에서는 운영 비밀번호를 표시하지 않습니다. 안내서에는 분석/세션리플레이/일반 앱 스크립트나 구조화 데이터가 없고, 설명서용 복사/인쇄 스크립트만 사용합니다.
- 보안 검사는 폐기 가능한 fixture 비밀번호로 표시·이스케이프·origin 경계를 확인합니다. 브라우저 검사는 복사 일치 여부를 불리언으로만 확인하며, 스크린샷/PDF를 저장하기 전에 DOM 비밀번호를 가립니다. 실제 비밀번호는 검사 로그나 산출물에 저장하지 않습니다.
- 비용·계좌: 15,000,000원 / 농협 1085-02-007634 / 문석준 / 입금 확인 후 2주 이내 세금계산서. ‘수정 기한은 영원히입니다’ 유지. 발송용 별도 원문·복사 패널 및 기술 점검 기록은 넣지 않습니다.
- 검증: typecheck/build 성공, 보안·표시 경계 11그룹 통과, 로컬 SEO 701페이지/이미지 36개 오류·경고 0, Chromium/Linux WebKit 각각 320·390·1440px에서 섹션 순서·비밀번호 표시/복사·결제·가로 넘침·인쇄·JS 오류 검사 통과. 시각 확인에서도 문서 레이아웃의 겹침/잘림이 보이지 않았습니다. CSS/JS v4.
- 운영: 앱 소스 `f33ce12`, 배포 `9340facc` (https://9340facc.seoul-dodam-dental.pages.dev), 정식 안내서 https://dodamdc.kr/handover 에 반영했습니다. 실주소에서 양대 브라우저 6뷰 모두 실제 비밀번호 표시/복사·레이아웃·결제·인쇄·분석 제외 검사를 통과했습니다. 값 자체는 로그/산출물에 기록하지 않았고 실제 환자정보에 접근하지 않았습니다.
- 운영 프록시가 자동 삽입하던 Cloudflare Insights 스크립트도 확인하여 이 문서에 `no-transform`과 제한적 CSP(`script-src 'self'`, `connect-src 'none'`)를 적용했습니다. 최종 운영 브라우저 검사에서 설명서 스크립트 외 추가 스크립트가 없음을 확인했습니다. 다른 페이지의 기존 분석/동작은 변경하지 않습니다.
...
- 기존 운영 `ADMIN_PASSWORD`·`SESSION_SECRET`은 유지합니다. 이 로그인 릴리스 당시에는 비밀번호를 공개하지 않았습니다. 이후 사용자의 명시적 공개 요청에 따른 현재 `/handover` 표시 범위는 위 최신 절을 따릅니다. 하단 기술 점검 기록을 관리자 접속 안내로 교체하고 결제 금액·계좌·지원 약속을 유지합니다.
...
- 검사는 폐기 가능한 D1/R2와 테스트 전용 비밀번호로 수행합니다. 운영에서 환자 등록/파기·사진 변경·비밀번호 재설정 시험을 하지 않습니다. typecheck/build, 보안 10그룹, 환자 동선 8그룹, Chromium/Linux WebKit 실제 비밀번호 전용 로그인 및 CMS 10그룹, 납품 안내 6뷰를 통과했습니다. 운영 앱 소스 `4af5ee8`, 배포 `e327be3d` (https://e327be3d.seoul-dodam-dental.pages.dev)로 기존 BYOK 프로젝트 main에 반영했습니다. 실주소 로그인 HTML에서 비밀번호 입력만 표시·초기 계정 생성 문구 없음·noindex/no-store·미인증 관리자 302와 핸드오버 접속/결제 안내를 확인했습니다. 실제 운영 비밀번호로 로그인하는 검사는 하지 않았으며 비밀번호 전용 성공 로그인은 격리 환경의 양대 브라우저에서 검증했습니다.
