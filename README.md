...
- 사용자 명시 요청대로 페이지와 복사용 발송 안내문 모두 **1,500만 원, 농협 1085-02-007634, 예금주 문석준, 입금 확인 후 2주 이내 세금계산서 발급**을 포함합니다. 부가세 포함 여부는 임의로 추가하지 않았습니다. 계좌·안내문 복사와 인쇄/PDF 기능을 제공합니다.
- 예시 병원의 비밀번호·미구현 기능·색인 수치·성과 보장을 가져오지 않았습니다. 기존 납품의 기술 검사 수치는 과거 점검 기록으로 구분합니다. 홈페이지·CMS·DB·환자 데이터는 이번 설명서 수정의 대상이 아닙니다.
- 운영 반영: 소스 `cbbd209`, 배포 `86f53e74` (https://86f53e74.seoul-dodam-dental.pages.dev), 정식 주소 https://dodamdc.kr/handover. Chromium/Linux WebKit 각각 320·390·1440px에서 결제 금액·농협 계좌·예금주·발송문 복사·인쇄·noindex·가로 넘침·JS 오류 검사를 통과했습니다. 실제 운영 HTML에서도 농협/문석준/1,500만 원/2주 이내 발급 문구를 확인했으며 부가세 포함 여부는 추가하지 않았습니다.
- 설명서 CSS/JS v2. noindex·사이트맵 제외·분석 제외는 유지합니다. 링크 열람자는 결제 안내를 볼 수 있으며, 이는 사용자가 명시적으로 요청한 공개 범위입니다.
