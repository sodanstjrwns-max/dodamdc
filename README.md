...
- **최근 변경**: 2026-09-06 승인 디자인의 전체 페이지 마감 보완 (타이포·FAQ·모바일 조작·목차·예약 화면)
...
## 전체 페이지 마감 보완 — 2026-09-06
- 기존 Wanted Sans, 블루·그린 팔레트, 입체 치아와 GSAP, 실제 사진, 진료 원문은 유지했습니다. 추가 라이브러리나 외부 API는 없습니다.
- 제목 줄바꿈과 본문 행간을 정리하고 진료 카드의 링크 위치, 장비 카드 여백, 사진 설명·주의 문구의 크기를 통일했습니다.
- FAQ의 Q/A 표기를 독립된 들여쓰기 영역으로 배치하고 열린 항목·키보드 포커스·본문 간격을 개선했습니다.
- 메뉴에 현재 페이지와 상위 메뉴를 표시하고 모바일 메뉴는 현재 진료가 포함된 하위 목록을 펼칩니다.
- 진료 상세의 빠른 목차는 스크롤 위치에 따라 활성 항목을 표시합니다. 고정 헤더와 목차 아래로 앵커가 정확히 이동하도록 중복 여백을 교정했습니다.
- 모바일 치아 회전·모션 토글·메뉴·상단 예약 버튼은 최소 44px 높이로 조정했습니다. 예약 단계, 약관 체크 상태, 입력 포커스, 푸터 링크도 보완했습니다.
- `test:design`의 공개 화면 80개 조합 및 추가 320/390/1440px 검사(조작 버튼 크기, FAQ 들여쓰기, 현재 메뉴, 목차 활성 상태, 앵커 위치)를 통과했습니다. 320/390/768px 예약 단계 정렬도 별도로 확인했습니다. 회전 버튼의 실제 44px 터치 영역은 SVG 폴백이 아닌 `test:kinetic`의 WebGL 화면에서 검증합니다.
- CSS와 기본 상호작용 스크립트의 캐시 버전은 `v=10`입니다. 미리보기에서만 반영하며 운영 배포는 별도 요청이 필요합니다.
