...
const guides: Record<string, { question: string; answer: string; checks: string[]; options: string; ask: string[]; equipment: string }> = {
  'vpt-crown': {
    ...
    equipment: '실제 MTA 재료·진단 장비 살펴보기',
  },
  periodontal: {
    ...
    equipment: '미온수 진료 환경·통증 배려 장비 살펴보기',
  },
  implant: {
    ...
    equipment: '실제 저선량 CT·진단 장비 살펴보기',
  },
}
...
export function consultationGuide(slug: string) {
  ...
  return html`<section class="consultation-guide" id="consultation-guide" aria-labelledby="consultation-title">
    ...
    <nav class="guide-links" aria-label="상담을 준비하는 관련 안내"><a href="/floor-guide">${g.equipment} ↗</a><a href="/doctors/han-hwirim">담당 의료진 소개 ↗</a><a href="/pricing">비급여 진료비·항목 확인 ↗</a><a href="/first-visit">첫 방문 준비물 확인 ↗</a><a href="/reservation?treatment=${slug}" data-conversion-location="consultation" class="btn btn-primary">이 진료 상담 신청</a></nav>
...
export function firstVisitPage(c: Context<Env>) {
  ...
  }, html`${pageHero({ eyebrow: 'YOUR FIRST VISIT', title: html`처음 오는 길이,<br>조금 더 편안하도록.`, lead: '치료 이름을 몰라도, 질문이 많아도 괜찮습니다. 방문 전에 알아두면 좋은 내용을 순서대로 정리했습니다.', crumbs: [{ name: '홈', href: '/' }, { name: '첫 방문 안내', href: '/first-visit' }], actions: html`<a href="#visit-steps" class="btn btn-outline">방문 순서 보기 ↓</a><a href="/reservation" class="btn btn-primary">예약 방법 확인</a>` })}
    <section class="section" id="visit-steps"><div class="container container-narrow"><h2 class="h2">예약부터 치료 계획까지, 여섯 걸음.</h2><ol class="visit-steps">${steps.map(([title, desc], i) => html`<li><span class="visit-step-number">0${i + 1}</span><div><h3>${title}</h3><p>${desc}</p>${i === 2 ? html`<nav class="guide-links" aria-label="내원 정보"><a href="/directions">오시는 길·주차 ↗</a><a href="/hours">진료시간 ↗</a><a href="/notice">공지사항 ↗</a></nav>` : ''}</div></li>`)}</ol>
