...
<ul><li><strong>회원가입</strong>: 이름, 이메일, 휴대전화번호, 비밀번호(암호화 저장). Google 계정으로 가입 시 Google이 제공하는 이름·이메일·프로필 식별자.</li><li><strong>진료 예약</strong>: 이름, 연락처, 이메일(선택), 희망 진료·일시, 문의 내용.</li><li><strong>자동 수집</strong>: 접속 경로(페이지 주소), 브라우저 종류(User-Agent), 접속 일시. IP 주소는 저장하지 않습니다.</li></ul>
...
<h2>3. 보유 및 이용 기간</h2>
<p>회원 정보는 회원 탈퇴 시까지, 예약 정보는 예약일로부터 1년까지 보유 후 파기합니다. 관계 법령에 따라 보존이 필요한 경우 해당 기간 동안 보관합니다.</p>
...
<h2>6. 쿠키</h2>
<p>로그인 유지를 위한 세션 쿠키(HttpOnly)를 사용합니다. 브라우저 설정에서 쿠키를 거부할 수 있으나, 이 경우 로그인 기능을 이용할 수 없습니다. GA4가 설정된 경우 익명화된 IP로 방문 통계를 수집합니다.</p>
<h2>7. 개인정보 보호책임자</h2>
...
export function sitemapHtml(c: Context<Env>) {
  const groups: [string, [string, string][]][] = [
    ...
    ['안내', [['/directions', '오시는 길'], ['/hours', '진료시간'], ['/pricing', '비급여 진료비'], ['/faq', 'FAQ'], ['/reservation', '진료 예약'], ['/area', '지역별 안내']]],
