...
export async function pricingPage(c: Context<Env>) {
  const clinic = c.get('clinic') as any
  // DB(fees) 공개 항목으로 렌더. 비었거나 실패하면 하드코딩 시드로 폴백 → 페이지가 절대 비지 않는다.
  const groups = (await loadPricingGroups(c.env?.DB, true)) ?? pricing
  const body = html`
...
<ul><li><strong>회원가입</strong>: 이름, 이메일, 휴대전화번호, 비밀번호(단방향 해시 저장). Google 계정으로 가입 시 Google이 제공하는 이름·이메일·프로필 식별자.</li><li><strong>진료 예약</strong>: 이름, 연락처, 이메일(선택), 희망 진료·일시, 문의 내용.</li><li><strong>자동 수집</strong>: 접속 경로(페이지 주소), 브라우저 종류(User-Agent), 접속 일시. IP 주소는 저장하지 않습니다.</li></ul>
...
<p>병원은 이용자의 개인정보를 제3자에게 제공하지 않습니다. 서비스 운영을 위해 다음 업체에 처리를 위탁합니다: Cloudflare, Inc.(웹 호스팅·데이터 저장), Google LLC(소셜 로그인, 웹 분석), Resend(예약 알림 이메일 발송).</p>
<h2>5. 이용자의 권리</h2>
<p>이용자는 언제든지 마이페이지 또는 병원 연락처를 통해 개인정보 열람·정정·삭제·처리정지를 요청할 수 있습니다. 회원 탈퇴 시 관련 정보는 지체 없이 파기됩니다.</p>
<h2>6. 쿠키</h2>
<p>로그인 유지와 요청 위조 방지를 위한 필수 쿠키(HttpOnly)를 사용합니다. 브라우저 설정에서 쿠키를 거부할 수 있으나, 이 경우 로그인 기능을 이용할 수 없습니다. GA4가 설정된 경우 익명화된 IP로 방문 통계를 수집합니다.</p>
<h2>7. 예약·문의 동선 집계</h2>
...
<p>예약 응대를 위해 담당 직원, 연락 시도·확인 상태, 연락 시각, 재연락 예정과 처리 이력을 관리합니다. 직원별 계정과 역할에 따라 접근을 제한하며, 비밀번호는 소금값을 적용한 단방향 해시로 저장합니다. 회원 예약 내역은 해당 회원 ID에 연결된 접수만 표시하며 이메일 주소가 같다는 이유만으로 연결하지 않습니다.</p>
