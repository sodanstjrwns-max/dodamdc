...
// Public rows remain private when all items are hidden. Only legacy installations
// without a fees table use the original schedule; transient failures must not revive stale fees.
...
async function fetchRows(db: any, publishedOnly: boolean): Promise<FeeRow[] | null> {
  ...
  try {
    ...
    if (String(e).includes('no such table: fees')) return null
    throw e
  }
}
