(() => {
  ...
  document.getElementById('handover-print')?.addEventListener('click', () => {
    const opened = [...document.querySelectorAll('.handover-guide-list details, .handover-reference:not(.handover-message)')].filter(el => !el.open);
  ...
  document.querySelectorAll('[data-copy-target]').forEach(button => button.addEventListener('click', async () => {
    ...
    try {
      ...
      status.textContent = target.id === 'delivery-message' ? '안내문 전체를 복사했습니다.' : '계좌번호를 복사했습니다.';
