...
(function(){var el=document.getElementById('clinic-status');if(!el)return;var hours;try{hours=JSON.parse(el.getAttribute('data-hours')||'[]')}catch(e){return}var hol=(el.getAttribute('data-holidays')||'').split(',');var D=['일','월','화','수','목','금','토'];function mins(t){var p=t.split(':');return (+p[0])*60+(+p[1])}
function calc(){var k=new Date(Date.now()+9*3600*1000);var ymd=k.toISOString().slice(0,10);var day=D[k.getUTCDay()];var cur=k.getUTCHours()*60+k.getUTCMinutes();var row=null;for(var i=0;i<hours.length;i++){if(hours[i].day===day)row=hours[i]}var holiday=hol.indexOf(ymd)>=0;
function nextOpen(){for(var i=1;i<=7;i++){var d=new Date(k.getTime()+i*86400000);var dd=D[d.getUTCDay()],r=null;for(var q=0;q<hours.length;q++){if(hours[q].day===dd)r=hours[q]}if(r&&r.open&&hol.indexOf(d.toISOString().slice(0,10))<0)return (i===1?'내일':dd+'요일')+' '+r.open+' 진료 시작'}return '진료 일정은 전화로 확인해 주세요'}
