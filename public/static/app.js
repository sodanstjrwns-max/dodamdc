...
/* 진료 중 표시 — 1분마다 KST 기준 갱신 (서버 초기값과 같은 규칙) */
(function(){var el=document.getElementById('clinic-status');if(!el)return;var hours;try{hours=JSON.parse(el.getAttribute('data-hours')||'[]')}catch(e){return}var hol=(el.getAttribute('data-holidays')||'').split(',');var subWed=(el.getAttribute('data-sub-wed')||'').split(',');var subRow=null;try{subRow=JSON.parse(el.getAttribute('data-sub-wed-hours')||'null')}catch(e){}var D=['일','월','화','수','목','금','토'];function mins(t){var p=t.split(':');return (+p[0])*60+(+p[1])}
/* 해당 날짜의 실제 시간표: 공휴일→null, 공휴일 주 수요일→평일 시간(서버가 계산한 data-sub-wed 목록) */
function rowFor(ymd,day){if(hol.indexOf(ymd)>=0)return null;var row=null;for(var i=0;i<hours.length;i++){if(hours[i].day===day)row=hours[i]}if(day==='수'&&!(row&&row.open)&&subRow&&subWed.indexOf(ymd)>=0)return subRow;return row&&row.open&&row.close?row:null}
function calc(){var k=new Date(Date.now()+9*3600*1000);var ymd=k.toISOString().slice(0,10);var day=D[k.getUTCDay()];var cur=k.getUTCHours()*60+k.getUTCMinutes();var row=rowFor(ymd,day);var holiday=hol.indexOf(ymd)>=0;
function nextOpen(){for(var i=1;i<=7;i++){var d=new Date(k.getTime()+i*86400000);var dd=D[d.getUTCDay()],r=rowFor(d.toISOString().slice(0,10),dd);if(r&&r.open)return (i===1?'내일':dd+'요일')+' '+r.open+' 진료 시작'}return '진료 일정은 전화로 확인해 주세요'}
if(holiday||!row||!row.open||!row.close)return{s:'closed',l:'진료 종료',d:(holiday?'공휴일 휴진 · ':day+'요일 휴진 · ')+nextOpen()};var o=mins(row.open),c=mins(row.close);if(cur<o)return{s:'closed',l:'진료 전',d:'오늘 '+row.open+' 진료 시작'};if(cur>=c)return{s:'closed',l:'진료 종료',d:nextOpen()};if(row.lunch){var lp=row.lunch.split(/[–-]/);var ls=mins(lp[0].trim()),le=mins(lp[1].trim());if(cur>=ls&&cur<le)return{s:'lunch',l:'점심시간',d:row.lunch.replace('–','~')+' · 이후 진료'}}return{s:'open',l:'진료 중',d:'오늘 '+row.close+'까지 진료 (접수 마감 30분 전)'}}
function render(){var r=calc();el.className='clinic-status is-'+r.s;el.title=r.d;var lb=el.querySelector('.status-label'),dt=el.querySelector('.status-detail');if(lb)lb.textContent=r.l;if(dt)dt.textContent=r.d}
